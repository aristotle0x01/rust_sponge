...
use rust_sponge::tcp_helpers::tcp_state::State::{CloseWait, LastAck, CLOSED, ESTABLISHED, RESET, LISTEN};
...
fn ack_listen_test(cfg: &TCPConfig, seqno: WrappingInt32, ackno: WrappingInt32, lineno: u32) {
    ...
    test.execute(&mut ExpectState::new(TCPState::from(LISTEN)), "".to_string());
    test.execute(&mut ExpectNoSegment{}, "test 3 failed: ACKs in LISTEN should be ignored".to_string());
}

fn ack_rst_syn_sent_test(cfg: &TCPConfig, base_seq: WrappingInt32, seqno: WrappingInt32, ackno: WrappingInt32, lineno: u32) {
    let mut test = TCPTestHarness::in_syn_sent(&cfg, base_seq);

    // any ACK should result in a RST
    test.send_ack(seqno, ackno, Option::None);

    test.execute(&mut ExpectState::new(TCPState::from(State::SynSent)), "".to_string());
    test.execute(&mut ExpectNoSegment{}, "test 3 failed: bad ACKs in SYN_SENT should be ignored".to_string());
}
...
fn fsm_ack_rst_relaxed() {
    ...
    // test #1: in ESTABLISHED, send unacceptable segments and ACKs
    {
        ...
        test_1.execute(&mut ExpectNoSegment{}, "test 1 failed: ACK after acceptable ACK".to_string());

        // ack in the past---no response
        test_1.send_ack(base_seq, base_seq - 1, Option::None);

        test_1.execute(&mut ExpectNoSegment{}, "test 1 failed: ACK after past ACK".to_string());

        // segment out of the window---should get an ACK
        test_1.send_byte(base_seq - 1, Option::Some(base_seq), 1);

        test_1.execute(&mut ExpectUnassembledBytes::new(0), "test 1 failed: seg queued on early seqno".to_string());
        ...
        // segment out of the window---should get an ACK
        test_1.send_byte(base_seq + cfg.recv_capacity as u32, Option::Some(base_seq), 1);

        test_1.execute(&mut ExpectUnassembledBytes::new(0), "test 1 failed: seg queued on late seqno".to_string());
        let mut one_seg1 = ExpectOneSegment::new();
        one_seg1.base_mut().with_ack(true).with_ackno(base_seq);
        test_1.execute(&mut one_seg1,
        ...
        test_1.execute(&mut ExpectData::new(), "test 1 failed: pkt not processed on next seqno".to_string());
    ...
    // test #2: in LISTEN, send RSTs
    {
        ...
        test_2.execute(&mut ExpectNoSegment{}, "test 2 failed: RST was not ignored in LISTEN".to_string());
    }
    ...
    ack_listen_test(&cfg, base_seq + cfg.recv_capacity as u32, base_seq, line!());
    ack_listen_test(&cfg, base_seq + cfg.recv_capacity as u32, base_seq + cfg.recv_capacity as u32, line!());

    // test 4: ACK and RST in SYN_SENT
    {
        println!("Test 4");
        let mut test_4 =
            TCPTestHarness::in_syn_sent(&cfg, base_seq);
        ...
        test_4.execute(&mut ExpectNoSegment{}, "test 4 failed: RST with good ackno should RESET the connection".to_string());
    }
