...
fn fsm_retx_relaxed() {
    ...
    // single segment re-transmit
    {
        ...
        test_1.execute(&mut Tick::new((cfg.rt_timeout - 2) as SizeT), "".to_string());
        test_1.execute(&mut ExpectNoSegment{}, "test 1 failed: re-tx too fast".to_string());

        test_1.execute(&mut Tick::new(2), "".to_string());
        check_segment(&mut test_1, &data, false, line!());

        test_1.execute(&mut Tick::new((10 * cfg.rt_timeout + 100) as SizeT), "".to_string());
        check_segment(&mut test_1, &data, false, line!());

        for i in 2..TCPConfig::MAX_RETX_ATTEMPTS {
            test_1.execute(&mut Tick::new(((cfg.rt_timeout << i) as u32 - i) as SizeT), "".to_string());  // exponentially increasing delay length
            test_1.execute(&mut ExpectNoSegment{}, "test 1 failed: re-tx too fast after timeout".to_string());
            test_1.execute(&mut Tick::new(i as SizeT), "".to_string());
            check_segment(&mut test_1, &data, false, line!());
        }

        test_1.execute(&mut ExpectState::new(TCPState::from(ESTABLISHED)), "".to_string());

        test_1.execute(&mut Tick::new((1 + (cfg.rt_timeout << TCPConfig::MAX_RETX_ATTEMPTS)) as SizeT), "".to_string());
        ...
        test_1.execute(&mut one_seg, "test 1 failed: RST on re-tx failure was malformed".to_string());
    }
}
